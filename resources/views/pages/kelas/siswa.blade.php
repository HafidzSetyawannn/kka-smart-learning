    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('siswas.create', ['kelas_id' => $kelas->id_kelas]) }}" class="btn bg-gradient-primary btn-sm">
                    ...
                    <table class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">No. Absen</th>
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex px-3 py-1">
                                            <h6 class="mb-0 text-sm">{{ $loop->iteration + $siswas->firstItem() - 1 }}</h6>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="align-middle text-center">
                                        <form action="{{ route('siswas.destroy', $siswa->id) }}" method="POST">
                                            <a href="{{ route('siswas.edit', $siswa->id) }}" class="btn btn-link text-secondary font-weight-bold text-xs">
                                                Edit
                                            </a>
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-link text-danger font-weight-bold text-xs" onclick="return confirm('Apakah Anda yakin ingin menghapus data siswa ini?');">
                                                Hapus
                                            </button>
                                        </form>
                                    </td>
